...
{
	public class RecyclerViewEditorCustomAutoCompletion : EditorPopupWindow
	{
		...
		public RecyclerViewEditorCustomAutoCompletion(CodeEditor editor) : base(editor, FeatureHideWhenFastScroll)
		{
			SetContentView(GetRoot());
			SubscribeToEditorEvents(editor);
		}

		public RecyclerViewEditorCustomAutoCompletion(CodeEditor editor, int features) : base(editor, features)
		{
			SetContentView(GetRoot());
			SubscribeToEditorEvents(editor);
		}
		...
		protected void OnEditorKeyEvent(Java.Lang.Object editorKeyEvent, Unsubscribe unsub)
		{
			...
			{
				if (IsShowing && listView.GetAdapter() != null && listView.GetAdapter().ItemCount != 0 && ev.CanIntercept())
				{
					ev.Intercept();
					try
					{
		...
		public virtual RecyclerView GetListView()
		{
			if (listView == null)
			{
				listView = new RecyclerView(Editor.Context);
				listView.SetLayoutManager(new LinearLayoutManager(Editor.Context)
				{
					Orientation = LinearLayoutManager.Vertical,
				});
				listView.SetAdapter(GetAdapter());
				((RecyclerViewCustomCompletionsAdapter)listView.GetAdapter()).ItemClick += (sender, e) => InsertCompletion(e.Position);
			}
		...
		public virtual RecyclerView.Adapter GetAdapter()
		{
			...
			adapter = new RecyclerViewCustomCompletionsAdapter(Editor, CompletionList)
			{
		...
		public virtual void InsertCompletion(int listPos)
		{
			...
			{
				Dismiss();
				var item = ((RecyclerViewCustomCompletionsAdapter)GetAdapter()).GetItem(listPos);
				Editor.RestartInput();
		...
		public void SetCompletions(IList<CompletionItem> items, bool show = true)
		{
			...
			((RecyclerViewCustomCompletionsAdapter)GetAdapter()).SetItems(items);
		...
		protected virtual void ApplyWindowSize()
		{
			...
			var height = Math.Min(itemHeight * adapter.ItemCount, maxHeight);
